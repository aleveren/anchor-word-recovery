...
do
    ...
    python3 uci_to_scipy.py docword.$corpus.txt M_$corpus.full_docs.mat
    echo "preprocessing: removing rare words and stopwords"
    python3 truncate_vocabulary.py M_$corpus.full_docs.mat vocab.$corpus.txt 50
    for loss in L2
    do
        for K in 20 50 100
        do
            echo "learning with nonnegative recover method using $loss loss..."
            python3 learn_topics.py M_$corpus.full_docs.mat.trunc.mat settings.example vocab.$corpus.txt.trunc $K $loss demo_$loss\_out.$corpus.$K
        done
    done
done
